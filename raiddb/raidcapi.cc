...
extern "C" SERVERDLL_API RaidDB* CreateRaidDB(const char* conn1, const char* container1, const char* conn2, const char* container2)
{
...
extern "C" SERVERDLL_API void DeleteRaidDB(RaidDB* raiddb) 
{
	delete raiddb;
}
...
extern "C" SERVERDLL_API void Close(RaidDB* raiddb) 
{
	raiddb->Close();
}

extern "C" SERVERDLL_API void Flush(RaidDB* raiddb)
{
	raiddb->Flush();
}

extern "C" SERVERDLL_API int Add(RaidDB* raiddb, int size, const char** keyptrs, const int* keylens, const char** valueptrs, const int* valuelens) 
{
	...
	for (int i = 0; i < size; i++) 
	{
		...
		data.push_back(std::make_pair(std::string(kptr, klen), std::string(vptr, vlen)));
	}
	return raiddb->Add(data).code();
}
	

extern "C" SERVERDLL_API int Get(RaidDB* raiddb, int size, const char** keyptrs, const int* keylens, char** valuesptr, int** valuelensptr)
{
...
void BuildScanResult(std::vector<std::pair<std::string, std::string>>& data, int* size, char** keysptr, int** keylensptr, char** valuesptr, int** valuelensptr)
{
...
extern "C" SERVERDLL_API int Scan(RaidDB* raiddb, const uint64_t token, int batchsize,
	int* size, char** keysptr, int** keylensptr, char** valuesptr, int** valuelensptr)
{
	std::vector<std::pair<std::string, std::string>> data;
...
extern "C" SERVERDLL_API int ScanPartialOrder(RaidDB* raiddb, const uint64_t token, int batchsize,
	int* size, char** keysptr, int** keylensptr, char** valuesptr, int** valuelensptr)
{
	std::vector<std::pair<std::string, std::string>> data;
...
extern "C" SERVERDLL_API void FreeScan(char* keysptr, int* keylensptr, char* valuesptr, int* valuelensptr)
{
	delete[] keysptr; 
	delete[] keylensptr;
	delete[] valuesptr;
